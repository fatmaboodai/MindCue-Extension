...
var spoilerTerms = []; // Array to store spoiler terms
...
String.prototype.replaceArray = function (find, replace) {
  ...
  if (find.length !== replace.length) {
    console.error("Find and replace arrays must have the same length.");
...
function loadSpoilerTermsFromStorage() {
  chrome.storage.sync.get(['spoilerterms'], function (result) {
    console.log("Loaded spoilerterms from storage");
    if (result.spoilerterms) {
      spoilerTerms = result.spoilerterms.map(function (term) {
        return term.toLowerCase();
      });
      replacer(); // Call replacer() inside the callback
    }
  });
}
  ...
  for (var i = 0; i < elements.length; i++) {
    ...
    for (var j = 0; j < element.childNodes.length; j++) {
      ...
      if (node.nodeType === 3) {
        ...
        var replacedText = text.replaceArray(spoilerTerms, ["*****"]); // Replace with whatever you want
...
// Call the initContentScript function to begin replacing words when the page loads
initContentScript();
